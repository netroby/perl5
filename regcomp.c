          ...
          )
{
		...
		if (!SIZE_ONLY) {
			...
			/* diag_listed_as: Can't find Unicode property definition "%s" in regex; marked by <-- HERE in m/%s/ */
                    ...
                    if (! is_invlist(prop_definition)) {
                        ...
                        /* We don't know yet what this matches, so have to
                        * flag it, and we certainly don't know enough to do
                        * optimizations */
     ...
     * like /./, expensive run-time lookups can be avoided.  Now that we have
